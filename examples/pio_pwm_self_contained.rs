// configures PIO State Mchines directly with Pins, does not interact with DMA, hence the bus/system
#![no_std]
#![no_main]
...
fn main() -> ! {
    ...
    let pio_program = pio_file!("./examples/pio_pwm_self_contained.pio", select_program("pio_pwm_self_contained"),);
    ...
    sm0.set_pindirs([(led_pin_id, hal::pio::PinDir::Output)]); // Theis GPIO pin are configured as an output.
